...
export default function Internships() {
  ...
  const internships = mockOpportunities.filter((opp) => opp.category === "Internship");

  const filteredOpportunities = internships.filter((opportunity) => {
      ...
      <main className="container py-8 space-y-8 animate-fade-in">
        <section className="text-center space-y-4 py-8">
          ...
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
            Find free internships to gain real-world experience
          </p>
        </section>
        ...
        <div className="text-muted-foreground">
          Showing {filteredOpportunities.length} {filteredOpportunities.length === 1 ? "internship" : "internships"}
        </div>
